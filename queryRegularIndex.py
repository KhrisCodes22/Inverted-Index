...
#test to see if shared folders working
#load the file
with open("indexRegular.json") as jsonFile:
    ...
    if(len(listOfQuery) == 1):
        ...
        if(len(docs) > 0):
            ...
            for x in range (0, len(docs)):
                positionList = ','.join(str(e) for e in invertedIndex[word][docs[x]])
                print(word + " is in document: " + docs[x] + " at positions: " + positionList)
            return
        else:
            printList(docs)
            return
